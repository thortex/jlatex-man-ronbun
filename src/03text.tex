%#!platex ron.tex
\chapter{文章の書き方}\label{chap:kousei}
\begin{abstract}
{\LaTeX}で文書を作成するためには文章の組版に関する約束事を
知る必要があります．論理的な文章を書きたいと思ったら，その
論理を知る必要があります．この章ではそれらを{\LaTeX}で実現する
ための基本的な部分を説明します．
\end{abstract}
...
様々なことを半自動的に行ってくれます．ここではその基本的な
約束を紹介します．

\section{表題}\label{subsubsec:maketitle}
...
\Cmd{date}\pa{日付}
\end{Syntax}
の三つを書き込みます．{\LaTeX}ではプリアンブルに表題の情報を
...
%\begin{Exe} \label{exe:maketitle1}\indindz{区切り}{著者の}%
...
%\begin{Exe} \label{exe:maketitle2}
%もう少し丁寧に著者名を紹介するとき，例えば
%\yo{氏名}，\yo{所属}，\yo{連絡先}の三つを記述したいときは
%\begin{InText}
%\author{未来太郎\\ 未来大学 未来学科\\ name@server.ac.jp}
%\end{InText}
%のようにします．このソースを実際に出力しその結果を吟味してください．
%例題~\ref{exe:maketitle1}の \cmd{thanks}命令と \cmd{and}命令を使って
%\begin{InText}
...
%\end{Exe}
%
%\begin{Prob} 以下のソースを出力してその結果を吟味してください．
%\begin{InText}
...
ても良いように通し番号をつけて一意的に管理します．

{\LaTeX}での見出しの定義は\tabref{table:headline}の通りです．%
\begin{table}[htpb]
\begin{center}
\caption{{\LaTeX}での見出しの定義}\label{table:headline}
...
\hspace*{3em}{\large \textbf{1.1 歴史的背景}}\\
{\Large \textbf{第2章 一般相対性理論}}\\
\hspace*{3em}{\large \textbf{2.1 電気学との関連}}\\
...
\caption{見出しの階層}\label{tab:sectiondepth}
\begin{tabular}{*3l}
\TR
\<見出し> & \<命令>               & \<深さ>${}^*$\\%$ 
\MR
...
これは20ページ以上の文書にあることが望まれます．目次といっても{\LaTeX}では
\begin{Syntax}
\Cmd{tableofcontents}
...
%実際の目次が出力されていないことに注目してください．端末には
%\begin{OutTerm}
% No file mokuji.aux.
% No file mokuji.toc.
% [1] (./mokuji.aux) )
%\end{OutTerm}
%というメッセージが表示されます．\dos{No File なんとか}という
%のは\va{なんとか}というファイルが存在しないよ，足りないよという
...
%\end{InText}
%という先ほどの\fl{mokuji.aux}とほとんど同じ記述があります．
...
%\end{OutTerm}
%のようなメッセージが表示されます．どうやら\fl{mokuji.aux}も
%\fl{mokuji.toc}も存在し，それらを適切に処理してくれたようです．
%出力ファイル\fl{mokuji.dvi}の先頭には始め出力されなかった\yo{目次}
%があることでしょう．
%\begin{OutText}
%{\large\headfont 目次}\\[1zw]{\small
%\hbox to 5zw{\textbf{1}\hfil}\textbf{ほげ}\hfill 1\\
%\hskip2zw\hbox to 3zw{1.1\hfil}ほげほげ？\dotfill~1\\
%\hbox to 5zw{\textbf{1}\hfil}\textbf{どれどれ}\hfill 1\\
%\hskip2zw\hbox to 3zw{1.1\hfil}どれどれどれ\dotfill~1}
%\end{OutText}
...
値を\tabref{tab:sectiondepth}に従って変更します．
...
値は\tabref{tab:sectiondepth}に従って変更します．
...
ません．そこで概要を\Z{章立て}すると良いので \Cmd{chapter*} 命令を使います．
このとき \cmd{chapter} 命令にアスタリスク\qu{\texttt*}をつけると目次に見出しを書き出さず，
章番号を付け足しません．使用例は%
\C*{addcontentsline}%
\begin{InText}
\chapter*{概要}\addcontentsline{toc}{chapter}{概要}
\end{InText}
と記述してから概要の文章を書きます．標準の文書クラスでは
概要専用のコマンドは定義されていません．
%用途は異なりますが
%\hito{奥村晴彦}の\Cls{jsbook}には\env{abstract}環境が定義
%されています．これは各章の始めに\yo{この章について}のような
%まえがきを書くときに使われます．

最後に\cls{(j)book}の場合ですが，これは \Cmd{frontmatter} が宣言
されているときに \cmd{chapter}命令を使うと余計な手間を省くことが出来ます．
具体的には\index{まえがき}%
\begin{InText}
\begin{document} 
\frontmatter%前付け
...
を必要とし、内閣が、その責任を負ふ。 
\end{OutText}
として自動的に字下げがなされます\footnote
   {『日本国憲法』 1947年5月3日\,施行の第1条から第3条までの引用．}．
明示的に \Cmd{par}命令
で段落の終了を知らせることができ，以下のようにも書けます．
\begin{InText}
天皇は、日本国の象徴であり日本国民統合の象徴であつて、
この地位は、主権の存する日本国民の総意に基く。\par
皇位は、世襲のものであつて、国会の議決した皇室典範の
...
\indindz{幅}{字下げの}%
字下げの幅は \Cmd{parindent} という長さ変数で
指定されているので
\begin{InText}
\parindent=3em 
\end{InText}
とすると約3文字分の字下げを段落の始めで行うことができます．


\subsection{行頭の字下げの有無}
\zindind{字下げ}{の抑制}%
\zindind{行頭}{の字下げ}%
\indindz{字下げ}{行頭の}%
段落の開始には字下げをすべきなのですが，
...
命令が使えます．
\begin{Syntax}
 \Cmd{indent}   可能ならば字下げをします．\\
 \Cmd{noindent} 可能ならば字下げをしません．
\end{Syntax}
...
\begin{InOut}
%\section{字下げ} 
\indent 私は\indent 大学生ですから，そうなります．\par
\noindent そうなりました．
\end{InOut}

%どうも最近のビジネス文書などでは字下げをせずに\zindind{字下げ}{の幅の調節}%
%段落と段落に\Z{空き}を入れて段落の終わりと段落の始まりを示す
%傾向があります．これを{\LaTeX}で行うためには
%段落と段落の空きを調節する \Cmd{parskip}という可変の長さ変数を
%調節して
%\begin{InText}
%\parindent=0pt%字下げ
%\parskip=10pt plus 0pt minus 0pt
%\end{InText}
%とします．実際にこのような設定にすれば分かりますが，
%ありとあらゆる部分に空きを入れてくれるので，
%別の方法を考えなければなりません．

%\subsection{段落の字下げ}\indindz{引用}{文の}\indindz{字下げ}{段落の}%
%文を引用している場合はそれが\Z{引用}であることを明確に
%するために，\emph{引用文全体を本文とは分け隔てる}習慣があります．
%これには\Env{quote}環境や\env{quotation}環境が
%使えます．ただしこの場合は自分で字下げ幅を
%設定できません．\env{quote}環境の中で使われている
%\Env{list}環境を自分でカスタマイズするとうまく
%行きます．もう少し簡単に行うには\Sty{indent}
%パッケージの\Env{indentation}環境を使います．
%\env{indentation}環境の\emph{一つ目}の必須引数には
%\emph{左側の}字下げ，\emph{二つ目}には\emph{右側の}字下げを指定します．
%\begin{InOut}
%ここは普通の文章領域です．不必要に字下げを
%調整するのは好ましいことではありません．
%\begin{indentation}{3zw}{3zw}
%左側の字下げは全角3文字分，右側の字下げ
%も全角3文字分ありますか？
%\end{indentation}
...
\pp{\tabref{tab:latexunit1}}は色々あります．ポイントは%
\indindz{長さ}{絶対的な}%
...
\end{center}
\end{table}
\hito{奥村晴彦}の\cls{jsclasses}ではクラスオプションに
...
単位の\Z{接頭語}として\tabref{tab:SI:prefix}の\Z{修飾子}が
使用できます%\footnote{%
%この他にも$10^{24}$から$10^{-24}$まで%$
%(Y Z P T G M k m \textmu\ n p f a z y) 
%ありますが，実際に用いられるだろう
%修飾子だけを掲載しました．}
．
\begin{table}[htbp]
 ...
 \Z{熱力学温度}& \Z{thermodynamic temperature} 
      &$T$&K & \Z{ケルビン}  &\Z{kelvin}\\
...
\Y{textcomp}パッケージの \C{textmu} コマンドを使います．
\end{footnotesize}
 \end{center}
\end{table}
%
...
右側（\Z{受け}）には入れていません．
%
%横書きのときは句読点は\emph{半角}のカンマとピリオドを使うのがベスト
%だと思います．その場合，\emph{句読点の後には半角のスペース}を入れてください．
%横書きで全角の句読点を使うときは
%\begin{OutText}
%今日のお弁当のふたには``I wish I could.''という文句が書かれていた。
%\end{OutText}
%のように二つの文書に異なる句読点が混在することになります．
%なるべく句読点は統一したほうが問題は少なくなります．
%ただし，
%\begin{OutText}
%今日のお弁当のふたには「I wish I could。」という文句が書かれていた。
%\end{OutText}
%などとするのはやめたほうが無難です．%横書きのときは文章の内容と
%相談して句読点を決めてください．
%
%句読点が少しおかしく見える例です．
%\begin{InOut}
...
%和文と欧文が混在する文書では欧文の場合には半角の句読点である`,'と`.'，
%和文には全角の`，'と`．'を用いると良いでしょう．
%本来ならばまず句読点のつけ方も引用の仕方も一つの文
%書の中で統一するが規則です．そのため和文と欧文が混在する場合は
%欧文の規則に従うということです．

%ですが，そうすると別の問題が出てきます．日本語組版で
%\yo{句読点は1文字分の幅を割り当てる}という規則に反す
%ることがあります．これには全角のコンマ\yo{，}とピリオド
%\yo{．}を使うようにすると我慢できます．
...
\end{Syntax}
レポート・論文の場合，\emph{傍注を使わずに脚注のみを使うようにしてくださ
い}．
...
\section{文字の強調}\zindind{下線}{の意味}
最近のワープロ文書では重要な文字列に\Z{下線} (\Z{underline})を引いて
\Z{強調} (\Z{emphasis}) を表現しているようです．論文や書籍では
\K{欧文をイタリック体}，\K{和文はゴシック体}にします．
\indindz{強調}{和文の}\indindz{強調}{文字列の}%
{\LaTeX}では文字列の強調のために \C{emph} 
が使えます．%もしかしたら使用している
%クラスファイルによっては和文がゴシック体になら
%ないかもしれません．
\begin{InOut}
欧文の強調には\emph{English Emphasize}と
して，和文の強調は\emph{文字列の強調}の
ようにします．
\end{InOut}

%場合によっては \Cmd{em}コマンドが使えます．これは
%\KY{宣言型コマンド}\pp{\secref{sec:ml:command}}と
%呼ばれるもので，広範囲な論理強調に使うことが出来ます．
%ただしこのコマンドを使うならば\KY{イタリック補正}を
%挿入する場合があります．イタリック補正とはイタリック体の
%文字の直後にローマン体などの別のフォントが来た場合に
%挿入すべき空白のことです．{\LaTeX}が用意している
% \Cmd{emph}や \Cmd{textit}などはこのイタリック補正
%が適切に挿入されるようになっています．宣言型の強
%調コマンド \Cmd{em}を使う場合は%
%\glossary{"/@\hspace*{-1.2ex}\verb+"\/+}%"
%自分で挿入しますから \cmd{/}命令を使うことになります．%"
%\begin{InOut}
...
%{\LaTeX}ではいくつかの半角の記号を直接出力することができません．
%これは\Z{予約文字}と呼ばれるもので，\qu{\texttt\bs}に似た
%命令の一種だと思ってください．出力できない記号は
%\begin{quote}
% \verb+\ { } $ & # ^ _ ~ % < > |+
%\end{quote}
%の13個であり，それらの記号を出力するには\secref{sec:chui}
%を参照してください．
%
...
%\begin{table}[htpb]
%\begin{center}
%\caption{直接出力できない半角記号}\label{tab:tokusyumoji}
...
\section{特殊記号}\indindz{記号}{特殊な}%
\Z{アクセント記号}などを出力するための\Z{特殊文字}も用意されており，
それらを出力するには\tabref{tab:tokusyukigou}の
...
%\end{InText}
\Z{アクセント}類を出力するには\tabref{tab:accent}の
命令を使います．
\qu{i}と\qu{j}にアクセントを付けるには\tabref{tab:tokusyukigou}中の
...
\index{シャープS}%
%
\caption{特殊記号}\label{tab:tokusyukigou}\indindz{記号}{特殊な}%
\zindind{点}{のないi}%
\zindind{点}{のないj}%
...
%"
% Dipl\^ome \'dAptitude Pratique au Fran\c{c}ais

%%\begin{InOut}
%%Fader v\r ar, som \"ar i himmelen.\par
%%I know Mr.\ M\"uller and Mr.\ K\"upper. 
%%\end{InOut}

%\subsection{日本の通貨記号}\indindz{記号}{円}
%\Z{円記号}\qu{\texttt{\yen}}を標準では出力することが
%できません．何らかのパッケージを読み込みます．
%\Sty{okumacro}や\Sty{ascmac}，
%\Sty{textcomp}などの既存のマクロではたいてい \cmd{yen}か
% \cmd{textyen}などの命令で定義されています．そもそもなぜ
%円記号が{\LaTeX}では出力できないのかをここではっきりと
%させておきましょう．
%
%Windowsユーザで{\LaTeX}を使い始めた人がまず疑問に思う
%ことの一つに円記号があります．普段自分の使っているエデ
...
%などを割り当てたり，\qu{\str{0x7E}}の\emph{バックスラッシュ}を\emph{円記
%号}に割り当てました．キーボードに円記号があるけれど実際に
...
たとえば\Z{プログラムリスト}を載せたいときは\Z{特殊記号}
...
\section{引用や文の区切り}\label{sec:quote}
...
一般的に以下のような使い方になります．
\begin{InOut}
`単語'の引用はシングルクオートで``文章の一
文''の引用は左シングルクオート二つと右シン
グルクオート二つです．"ダブルクオート"で
引用符を表してはいけません．
\end{InOut} 
\begin{InOut}
段落を引用するquote環境の他にも
...
%\subsection{欧文と和文の引用の違い}
和文の引用における\KY{引用符}は\K{全角の\Z{かぎ括弧}}\str{「」}を
使い，欧文の場合の引用符には\K{半角のクオート}\str{`'}を使います．
和文の引用の中の引用には\Z{二重括弧}を用います．和文の場合，
\K{括弧の中に句点を入れてはいけません}．
%一つの文書に両者が混在するのは余り好ましい状態とは言えません．
%本来の和文とは言葉の意味は\emph{漢文}に対する
%\emph{日本語}という意味であって仮名文字を示しま
%すが，本書では仮名漢字まじりの日本語を意味します．
\begin{InOut}
``\,FUN: Future University-Hakodate''
は恐らく`FUNNIST'との密接な関わりがあり，
渡辺によると「未来らによると『FUNNISTはFUNに
ある組織である』という説がある」と考察して
いる．
\end{InOut}
%どちらか一方に統一するのが作成者側にも読者にも混乱は少ないでしょう．
%{\LaTeX}で引用というものをもう少し効率良く行うには
%\begin{InText}
...
%\end{InText}
%のような命令を定義しておけば，後から引用符
%を統一できます．上記の \cmd{yo}と \cmd{yy}を
%\begin{InText}
%\newcommand{\yo}[1]{`#1'}% 単語の和文引用 
%\newcommand{\yy}[1]{``#1''}% 1 文の和文引用 
%\end{InText}
%と変更するだけで文章中の引用符を一括して
%変更できるわけです．
%\begin{InOut}
...
\begin{Syntax}
 \Cmd{emph}\pa{article's name} \pp{欧文の場合}\\
 『書籍名』\pp{和文の書籍} \indindz{書籍}{和文の}\\
 「雑誌名」\pp{和文の雑誌} \indindz{雑誌}{和文の}
\end{Syntax}
以上のような方法を使って何か別の文書を示す場合はその文書名
を強調表示します．
\begin{InOut}
渡辺が2004年に\emph{Natural}に投稿した論文
「論文作成のいろは」は未来出版から『論文作
成の手引き』に改題されて出版されている．
\end{InOut}
...
%\end{InText}
%\begin{InOut}
%渡辺が2004年に\yousyo{Natural}に投稿した論文
%\wasyo{それが私の生きる道}には「人生そんなもの
%だよ．」という名言が書かれている．
%\end{InOut}
...
\index{ダーシ!波\zdash}%
   ...
   使う例も見られますが，「10〜30人」という表記は\K{避けた方が
   無難です}．
\index{"-"-"-@\verb+---+}%%"
 \item[\Z{em-dash} \qu{---}] 
   文の中断を表します．
...
\indindz{記号}{横棒の}%
以上の記号を混同することなく正しく使うのが好ましいです．
倍角ダーシを出力するためには\Sty{okumacro}パッケージを
読み込みます．出力方法は\tabref{tab:dash}の通りです． 
%
\begin{table}[htbp]
\begin{center}
 \caption{ダッシュなど}\label{tab:dash}
 \begin{tabular}{*4l}
 \TR
 \<記号の種類>   & \<出力>  & \<入力・命令>   & \<用法>\\
 \MR
...
\end{table}
\begin{InOut}
``When I was a dog---a big dog---I could 
read about 100--200 books in a day. 
...
改行はしませんので，\C{mbox} 命令でくくります．これは通常の1単語のハイフ
ネーションと重複する可能性があるからです．\indindz{改行}{ハイフン途中の}
\begin{InOut}
{\TeX}の\mbox{for-each}文はPerlにおける
\mbox{foreach}文とは性質が異なるた
め，\mbox{X-ray}の影響を受けた
Future \mbox{University-Hakodate}は
\mbox{if-then}文を使う傾向にある．
\end{InOut}

%\subsection{ハイフネーション}
%行末に長い単語がある場合にその単語のハイフネーションに
%あった改行位置を見つけて{\LaTeX}は改行を行います．ユーザーが明示的に
%改行位置を指定することもできます．\index{改行!のいち@\zdash の位置}
%\begin{Syntax}
%\Cmd{hyphenation}\pa{複数の語句} \\
%\Cmd{-} \pp{位置指定}
%\end{Syntax}
% \cmd{-}は文章中で直接\qu{\str{Ghost\bs-script}}のように使います．
...
%\end{InOut}
%\qu{superなんちゃら}の一つ目，二つ目では改行の位置が違
...
引用\pp{\secref{sec:quote}参照}を使うとうまく行くことが多いです．
\begin{Syntax}
\verb|\\*|\opa{長さ}\\
\Cmd{newline} \\
\Cmd{par}
...
以下の入出力を見比べてください．
\begin{InOut}
The files were found in a folder and 
were shuffled by anyone.\par
The f{}iles were found in a folder and 
...
\K{単語間空白}と呼びます．これは人間が意図的に単語の区切り
として`\verb*|My name is Thor.|'のように挿入します．
...
N.Y. I wish I could. D.~E. Knuth 
means `Donald Ervin Knuth.'
\end{InOut}
%分かりづらいですが
...
最後に文章における空白をまとめると次の五つがあるということです．
\index{space}%
...
\zindind{改行}{の抑制}%
それらが並んでいることで一つの意味を持つ単語間には改行を入れ
ないようにします．例えば人名やページ番号，略語などは，一ま
...
\end{InOut}
\Z{引用符}が隣り合うときには，引用符と引用符のあいだに小さな空白を挿入します．
\begin{InOut}
```Hello' is a greeting and I always 
say `Hello.''' \par ``\,`Hello' is a 
...
\section{箇条書き} 
\Z{箇条書き} (\Z{itemization}) には三つの環境を使うことが出来ます．
\indindz{記号}{箇条書きの}%
\begin{description}
...
これらの環境は\KY{入れ子} (\Z{nest}) にすることが可能です．
\Z{入れ子}に出来る項目の深さは通常\K{四つまで}です．
\texttt{itemize}環境
...
\begin{Prob}
ここまでの節を読んだだけでも，ある程度の規模の文書を作成することが
出来ます．次のような\Va{file}{tex}を作成し，実際にタイプセッ
...
\begin{quote}
段落ごとの引用の場合は\verb|quotation|環境を使い、\K{行頭
...
\section{行揃え}
%日本人は行揃えを良く使いたがるそうです．
\KY{行揃え} (\Z{flushing}) には三つの環境と三つの宣言型コマンドを
使うことが出来ます
\pp{\tabref{tab:hss}}．
\begin{table}[htbp]%{l}{21zw}
\begin{center}
   \caption{揃えの命令と宣言}\label{tab:hss}
  \begin{tabular}{*3l}
   \TR
   \<種類>     & \<環境> & \<宣言>\\   
   \MR
   左揃え   &\Env{flushleft}  & \C{raggedright} \\
   中央揃え &\Env{center}     & \C{centering} \\
   右揃え   &\Env{flushright} & \C{raggedleft} \\
   \BR
  \end{tabular}
\end{center}
\end{table}
環境型のコマンドは広い範囲に使い，宣言型のコマンドは
一つの要素や別の環境の中で使うことができます．
\Z{中央揃え} (\Z{centering})には\Env{center}環境です．
1行もしくはそれ以上の文字列，表，図などを中央に寄せるこ%
\index{改行!きようそろえにおける@行揃えにおける\zdash}%
とが可能です．\K{行頭や最終行に改行は入れません}．
\Z{右揃え} (\Z{flushright}) には\Env{flushright}環境です．
文字列を右寄せにします．
\Z{左揃え} (\Z{flushleft}) には\Env{flushleft}環境です．
字下げを行わずに左に寄せます．
%
\begin{InOut}
ビジネス文書で大活躍するでしょう．
...
世の中にはこれらを書体というひとつの枠組みで整理してい
ます．書体は読者に対して何らかのメッセージを分かりやす
く伝えるために変更される場合があります．ですから書体を
変更するということには必ず意味があるべきなのです．むや
みやたらに書体を変更しても逆に読者を混乱させます．また
自分だけのルールで書体を変更しても読者には何の意味なの
かが分かりませんので，一般的に使われている書体に関する
ルールを守るのもマナーです．

{\LaTeX}は\Z{マークアップ}型のシステムなのでユーザーが
直接書体変更用の命令を使うことは本来ならば必要のないこ
とだと思われます．以下のコマンドは直接使うのではなく新
規に環境を定義して用いるのが望ましいでしょう．

\subsection{文字の大きさの変更}
{\LaTeX}においては比較的簡単に文字の大きさを変える
ことが可能ですが，文字は文書クラスオプションで指定
した基準の文字の大きさに応じて変更されます ．文字
の大きさを変更したいときは\tabref{tab:mojinookisa}
の宣言型のコマンドを
\begin{quote}
\verb+{\命令 文字の大きさを変えたい文字列}+
\end{quote}
のように使用します．
\begin{table}[htbp]
\begin{center}
\caption{文字の大きさの変更}\label{tab:mojinookisa}
\begin{tabular}{*3l}
\TR
\<大きさ>       & \<命令>                 & \<出力例>\\
\MR
...
\label{tab:moji:ookisatte}
 \begin{tabular}{l*3rl}
 \TR
 \<コマンド>\texttt\bs \<基準の大きさ>& \<10\,pt>  & \<11\,pt>   & \<12\,pt> &
 \<使用すべき要素>${}^{*}$\\
\MR
\C{tiny}        & 5\,pt  & 6\,pt  &  6\,pt & 振り仮名\\
\C{scriptsize}  & 7\,pt  & 8\,pt  &  8\,pt & \\
\C{footnotesize}& 8\,pt  & 9\,pt  & 10\,pt & 索引・\Z{脚注}\\
\C{small}       & 9\,pt  & 10\,pt & 11\,pt & 図表見出し\\
\C{normalsize}  & 10\,pt & 11\,pt & 12\,pt & 小小節見出し・本文\\
\C{large}       & 12\,pt & 12\,pt & 14\,pt & 小節見出し\\
\C{Large}       & 14\,pt & 14\,pt & 17\,pt & 節見出し\\
\C{LARGE}       & 17\,pt & 17\,pt & 20\,pt & \\
\C{huge}        & 20\,pt & 20\,pt & 25\,pt & 部・章見出し番号\\
\C{Huge}        & 25\,pt & 25\,pt & 25\,pt & 部・章見出し\\
\BR
\end{tabular}
\par\vskip1ex
...
%
\begin{InOut}
そういえば，{\scriptsize これ}は小さい文字だ
けど，{\Large こっち}は大きい文字になってるね．
\end{InOut}
...
\subsection{書体の変更}\label{subsec:ouyou:font}
...
%文字の大きさや書体の種類を不必要に変更することは，
%逆に読者を混乱させるだけです．
ローマンファミリーは本文の書体に使います．
サンセリフファミリーは見出しなどに使うべきですが，
近年の公式な文書においてもサンセリフファミリーが本文に
使われることもあります．タイプライタファミリーは
プログラムのコードを示す場合に使われます．

\tabref{tab:font:family}の一覧から適切な書体を
選んでください．
\index{ローマン体}%
\index{サンセリフ体}%
\index{タイプライタ体}%
...
\begin{table}[htbp]
\begin{center}
\caption{書体を変更するコマンド}\label{tab:font:family}
\begin{tabular}{*4l}
\TR
\<種類> & \<命令> & \<宣言> & \<出力> \\
\MR
...
和文の書体は基本的には\Z{明朝体}と
\Z{ゴシック体}の二つしか用意されてい
ません\pp{\tabref{tab:font:wa}}．これは
従来の和文組版で二つの書体しか使われな
かった名残です．現在の{\pLaTeX}で和文
の多書体を図ることはそれ程難しくあり
ません．ただ不用意に和文を多書体にして
も読者がそれに慣れていないと思われます
ので，悪戯に行わないほうが良いかもしれません．
\begin{table}[htbp]
\begin{center}
\caption{和文書体のファミリー}\label{tab:font:wa}
  \begin{tabular}{*4l}
\TR
 \<種類> & \<命令> & \<宣言> & \<出力>\\
\MR
...
ゴシック体は\textgt{文章の強調に}使われま
す．{\gtfamily 見出しも強調すべき要素なの
でゴシック体にするのが普通です．}
\end{InOut}



\section{文章の修正}\zindind{原稿}{の校正}%
このようにして基本的な文章の論理構造を組み上げて，結果的に紙
の上などに出力するわけですが，一発で完璧な文書になることはほ
...
\section{クラスとパッケージ}\label{sec:class}
{\LaTeX}は\Z{マークアップ}言語なので\Z{書式}と内容は
分離されるのが普通です．そこで\Z{クラス}\pp{\Z{class}}と
...
\zindind{マクロ}{パッケージ}%
を集めたものを\K{パッケージ}と呼びます．パッケージは\K{マクロ
パッケージ}とか，ただ単に\KY{マクロ}などと呼ばれます．
...
パッケージの拡張子には\exten{sty}を付けるようにしています．

\subsection{標準的なクラス}\label{01pregame:sec:classes}

\indindz{クラス}{標準的な}%
{\LaTeX}や{\pLaTeX}の範囲内で提供されている標準的なクラスを
紹介します．クラスファイルは\Va{classes}{dtx}と
\Va{classes}{ins}という二つのファイルで配布されることが多い
ようです．
...
%\end{TYPE}
%とするとそのクラスを導入することができます．このような操作をすると
...
分かれます．標準では\Cls{article}，\Cls{report}，\Cls{slides}，
\Cls{proc}が使えます．
%\begin{namelist}{xxxx}
...
%	開発者用のクラスファイル．一般ユーザーにはあまり縁のないもの．
...
\Cls{jclasses}と呼ぶことがあります．

\subsection{クラスオプション}\label{sec:classopt}
ドキュメントクラス\pp{文書クラス，または単に\Z{クラス}}には
もう少し詳細な設定を行うことができます． \cmd{documentclass}の
任意引数として記述します．多くのクラスファイルでは次の
\Z{クラスオプション}が使えると思います．
\begin{description}
...
\Z{一段組}\pp{\option{onecolumn}}にするか\Z{二段組}\pp{\option{twocolumn}}
にするかを指定します．
...
標準は\option{final}．\indindz{原稿}{ドラフト段階の}%
...
最近では，\Hito{奥村晴彦}が管理している\Cls{jsclasses}
というクラスファイル群が定評です．このクラス群を導入すると，
\begin{namelist}{xxxxxx}
\item[\Cls{jsarticle}]	
小規模の日本語の記事を作成するためのクラス．

\item[\Cls{jsbook}]		
日本語の書籍や報告書を作成するためのクラス．

\item[\Cls{jspf}] \indindz{クラス}{学会誌用の}
某学会誌用のクラス．
\end{namelist}
...
\Z{レポート作成}用に \cmd{chapter} 命令を使う事が出来ます．
\cls{jsbook} では左右起し等に関する設定が変わります．
\end{description}


\subsection{標準で使用できるパッケージ}\label{01pregame:sec:stdmacro}
...
\end{TYPE}
とすれば\Va{filename}{dvi}が作成されます．索引の作成や
目次の作成，相互参照の解決などをすれば完全なDVIファイルが
完成します．各ソースファイルへの検索パスがなければ
該当する\Va{filename}{dtx}を検索することはできません．
...
\item[{\Prog[AmSLaTeX]{\AmSLaTeX}}] 
  ...
  {\AmSLaTeX}で，パッケージの名前は\Y{amsmath}と言う．
  数学系の文書を書くときには必須のマクロ．
  \indindz{マクロ}{数学系の}%
\item[\Sty{babel}] 多言語を{\LaTeX}で扱うためのマクロ．
  このマクロを日本語と共存させるためには少々工夫が必要．
%  詳しい情報は付録の\ref{subapp:webpage}~節を参照してください．
%\item[\Sty{cyrillic}]キリル書体で文書を作成するときに必要になるマクロ群．
\item[\Sty{graphicx}] 画像の挿入や加工などを担うマクロ．
同時に\Sty{color}というマクロも含まれる．これは\Z{デバイス}
（\Z{装置}）依存の機能で環境により出力が異なることがある．
%\item[\Sty{psnfss}] Everything you need for typesetting with a large range of Type 1 fonts．
\item[\Sty{tools}] \LaTeX3プロジェクトチームによって提供される
標準からは外されたマクロ．
\end{description}

これらのマクロについては少なくとも\wasyo{\COMP}か\wasyo{\LMANUAL}
に記述されていることが保証されています．

{\LaTeX3}プロジェクトチームによって提供される\sty{tools}
